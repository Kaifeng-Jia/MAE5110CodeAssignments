"""Controlled inverted pendulum walker model and visualization."""
...
def generate_params():
    """Return the model parameters and default controls."""
    return {
        "gravity": 9.81,  # m/s^2
        "length": 1.0,  # m
        "mass": 1.0,  # kg
...
def dynamics(t, state, params):
    """Calculate the continuous state derivative."""
...
def event_guard(previous_state, next_state, params):
    """Detect forward foot contact."""
    contact_angle = params["incline"] + params["angle_of_attack"]
    return previous_state[0] < contact_angle <= next_state[0]


def event_dynamics(state, params):
    """Reset the state after changing support legs."""
    angular_velocity = state[1]
    angle_of_attack = params["angle_of_attack"]
    incline = params["incline"]
    next_angle = incline - angle_of_attack
    next_angular_velocity = angular_velocity * np.cos(2 * angle_of_attack)
    return np.array([next_angle, next_angular_velocity])
